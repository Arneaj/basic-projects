...
global fig_is_closed::Bool = false
...
function shade(some_colored_world, 
          some_height_map, 
          xL, yL, hL, dl)
...
function shade_threaded_simple(some_colored_world, 
    some_height_map, 
    xL, yL, hL, dl)
...
function on_a_press(event)
    global fig_is_closed = true
end
...
function animated_plot_mouse_sun()
    ...
    mouse_move = fig.canvas.mpl_connect("motion_notify_event", mouse_mvmt)

    close_fig = fig.canvas.mpl_connect("key_press_event", on_a_press)

    while true
        xL = xSun
        yL = ySun

        temp_shaded_world = shade(colored_map, height_map, yL, xL, 3, 0.001)
        ...
        if fig_is_closed break end
    end

    plt.close()

    fig.canvas.mpl_disconnect(mouse_move)
    fig.canvas.mpl_disconnect(close_fig)
end
